...
sexos = st.sidebar.multiselect("Sexos: ", ["Masculino", "Femenino"])
Ano = st.sidebar.multiselect("Ano: ", df["Ano"].unique())
Mes = st.sidebar.multiselect("Mes: ", df["Mes"].unique())
Nacionalidad = st.sidebar.multiselect("nacionalidad: ", pd.unique(df[['RD', 'Haiti', 'Otra nacionalidad', 'Sin datos de la nacionalidad']].values.ravel()))
Tipo_de_Servicio = st.sidebar.multiselect("Tipo de Servicio: ", df["Tipo de Servicio"].unique())
Seguro= st.sidebar.multiselect("Seguro: ", df[["Senasa", "Otros", "Sin datos del seguro"]].values.ravel())
if "Nombre del servicio" in df.columns:
    Nombre_del_servicio = st.sidebar.multiselect("Nombre del servicio: ", df["Nombre del servicio"].unique())
else:
    st.sidebar.write("La columna 'Nombre del servicio' no existe en los datos.")
    Nombre_del_servicio = []
